    ...
    private function buildIssues(
        ...
        if ($incompleteOrderCount > 0) {
            $issues[] = array(
                ...
                'action' => 'جزئیات هزینه خرید و اقلام سفارش را بررسی و داده ناقص را تکمیل کنید.',
            );
        }
        ...
        if ($productsWithMissingCogs > 0) {
            $issues[] = array(
                'severity' => 'warning',
                'title' => 'اطلاعات هزینه خرید کامل نیست',
                ...
                'action' => 'از صفحه سودآوری محصولات، محصول‌های ناقص را باز و هزینه خرید را تکمیل کنید.',
            );
        }
        ...
        if ($refundWarningCount > 0) {
            $issues[] = array(
                'severity' => 'warning',
                'title' => 'بازگشت وجه نیازمند بازبینی وجود دارد',
                'message' => 'برای بعضی بازگشت‌های وجه، تخصیص مبلغ یا بازیابی هزینه خرید کاملاً قابل تطبیق نبوده است.',
                'metric' => number_format_i18n($refundWarningCount) . ' سفارش',
                'action' => 'جزئیات بازگشت وجه همان سفارش را در مرکز سفارش‌ها بررسی کنید.',
            );
        }
        ...
        if ($issues === array()) {
            $issues[] = array(
                ...
                'message' => 'مشکل مهمی در هزینه خرید، ارز، بازگشت وجه، قابلیت محاسبه یا آمادگی داده جغرافیایی دیده نشد.',
                'metric' => 'آماده تحلیل',
